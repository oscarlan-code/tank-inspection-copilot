package ai.laiq.tankinspection.presentation.v2product.taskhome
...
import ai.laiq.tankinspection.v2product.storage.V2TaskLifecycle
...
data class V2TaskHomeUiState(
    val isLoading: Boolean = true,
    val profile: V2LocalProfile? = null,
    val tasks: List<V2TaskSummary> = emptyList(),
)

data class V2ExportReviewUiState(
    val task: V2TaskSummary,
    val isLoading: Boolean = true,
    val isExporting: Boolean = false,
    val validationChecks: List<V2ExportValidationCheck> = emptyList(),
    val latestExportPackage: V2ExportPackageSummary? = null,
...
@Composable
fun V2TaskHomeScreen(
    state: V2TaskHomeUiState,
    exportReviewState: V2ExportReviewUiState?,
    onStartNewInspection: () -> Unit,
    onContinueInspection: (V2TaskSummary) -> Unit,
    onReviewExport: (V2TaskSummary) -> Unit,
    onDismissExportReview: () -> Unit,
    onExportInspection: (V2TaskSummary) -> Unit,
    onArchiveInspection: (V2TaskSummary) -> Unit,
    onDeleteInspection: (V2TaskSummary) -> Unit,
    onSaveProfile: (tenantName: String, workspaceName: String, displayName: String) -> Unit,
    contentPadding: PaddingValues = PaddingValues(0.dp),
) {
    var archiveTarget by remember { mutableStateOf<V2TaskSummary?>(null) }
    var deleteTarget by remember { mutableStateOf<V2TaskSummary?>(null) }
    var showProfileDialog by remember { mutableStateOf(false) }
    val profile = state.profile
    val ongoingTasks = state.tasks.filter { task -> task.lifecycle == V2TaskLifecycle.ONGOING }
    val archivedTasks = state.tasks.filter { task -> task.lifecycle == V2TaskLifecycle.ARCHIVED }
    ...
    Surface(
        ...
        Column(
            ...
            Row(
                ...
                if (task.lifecycle == V2TaskLifecycle.ONGOING) {
                    LaiqPrimaryButton(
                        text = "Continue",
                        onClick = onContinue,
                        modifier = Modifier.weight(1f),
...
@Composable
private fun LatestExportCard(summary: V2ExportPackageSummary) {
    Surface(
        modifier = Modifier.fillMaxWidth(),
        color = Color.White,
        shape = MaterialTheme.shapes.large,
...
@Composable
private fun ValidationCheckCard(check: V2ExportValidationCheck) {
    Surface(
        modifier = Modifier.fillMaxWidth(),
        color = Color.White,
        shape = MaterialTheme.shapes.large,
