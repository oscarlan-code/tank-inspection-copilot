...
enum class V2WorkflowScreen(val key: String, val label: String) {
    ...
    companion object {
        fun fromKey(key: String): V2WorkflowScreen =
            entries.firstOrNull { screen -> screen.key == key } ?: GENERAL_INFO
    }
}

enum class V2TaskLifecycle(val key: String) {
    ONGOING("ongoing"),
    ARCHIVED("archived"),
}

data class V2LocalProfile(
    val tenantId: String,
...
data class V2TaskSummary(
    val inspectionId: String,
    val inspectionReference: String,
    val client: String,
    val tankNumber: String,
    val location: String,
    val lifecycle: V2TaskLifecycle,
    val currentScreen: V2WorkflowScreen,
...
data class V2ExportValidationCheck(
    val ruleCode: String,
...
data class V2ExportPackageSummary(
