package ai.laiq.tankinspection.presentation.v3product.finding
...
import ai.laiq.tankinspection.v3product.model.withPhotoAnnotations
import ai.laiq.tankinspection.v3product.model.withRemovedPhoto
import ai.laiq.tankinspection.v3product.model.withSelectedPhoto
...
@Composable
fun ProductFindingCaptureScreen(
    record: ProductFindingRecord,
    allFindings: List<ProductFindingRecord> = emptyList(),
    imageRootDir: File,
    onRecordChange: (ProductFindingRecord) -> Unit,
    onSelectFinding: (ProductFindingRecord) -> Unit = {},
    onDeleteFinding: (ProductFindingRecord) -> Unit = {},
    onTakePhoto: () -> Unit,
    onRetakePhoto: (ProductFindingPhoto) -> Unit = {},
    onImportPhoto: () -> Unit,
    onBack: () -> Unit,
    ...
    var deleteTarget by remember { mutableStateOf<ProductFindingRecord?>(null) }
...
private fun PreviousFindingsStrip(
    findings: List<ProductFindingRecord>,
    expanded: Boolean,
...
private fun AnnotatablePhoto(
    ...
    imageRootDir: File,
    onAnnotationChange: (List<ProductAnnotationStroke>) -> Unit,
) {
    val latestPhoto by rememberUpdatedState(photo)
    var canvasSize by remember(photo.id) { mutableStateOf(Size.Zero) }
    var activeStroke by remember(photo.id) { mutableStateOf<List<ProductAnnotationPoint>>(emptyList()) }
    ...
    Surface(
        ...
        Box(
                    ...
                    detectDragGestures(
                        ...
                        onDragEnd = {
                            if (activeStroke.size >= 2) {
                                onAnnotationChange(latestPhoto.annotationStrokes + ProductAnnotationStroke(activeStroke))
                            }
            ...
            Canvas(modifier = Modifier.fillMaxSize()) {
                (photo.annotationStrokes + ProductAnnotationStroke(activeStroke).takeIf { activeStroke.size >= 2 })
                    .filterNotNull()
                    .forEach { stroke ->
...
private fun ProductAnnotationPoint.toOffset(
    containerSize: Size,
    bitmap: Bitmap?,
): Offset {
    val imageBounds = fittedImageBounds(containerSize, bitmap)
