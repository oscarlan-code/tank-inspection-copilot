...
enum class ProductChecklistRating(
    val key: String,
    ...
    companion object {
        fun fromKey(key: String?): ProductChecklistRating? =
            entries.firstOrNull { rating -> rating.key == key }
    }
}

data class ProductChecklistItemDefinition(
    val number: Int,
    val prompt: String,
)

data class ProductChecklistSectionDefinition(
    val key: String,
    val title: String,
    val items: List<ProductChecklistItemDefinition>,
    val commentsHint: String? = null,
)

data class ProductInspectionChecklistState(
    val selectedSectionKey: String = ProductInspectionChecklistCatalog.sections.first().key,
    val ratingsByItemNumber: Map<Int, ProductChecklistRating> = emptyMap(),
    val itemNotesByItemNumber: Map<Int, String> = emptyMap(),
    val sectionComments: Map<String, String> = emptyMap(),
)

object ProductInspectionChecklistCatalog {
    val sections: List<ProductChecklistSectionDefinition> = listOf(
        ProductChecklistSectionDefinition(
            key = "diked_area",
            title = "Diked Area",
            items = listOf(
    ...
    fun sectionByKey(key: String): ProductChecklistSectionDefinition =
        sections.firstOrNull { section -> section.key == key } ?: sections.first()
}

fun ProductInspectionChecklistState.withSelectedSection(sectionKey: String): ProductInspectionChecklistState =
    copy(selectedSectionKey = sectionKey)

fun ProductInspectionChecklistState.withRating(
    itemNumber: Int,
...
fun ProductInspectionChecklistState.withSectionComment(
    sectionKey: String,
    comment: String,
): ProductInspectionChecklistState =
    copy(
...
fun ProductInspectionChecklistState.withItemNote(
    itemNumber: Int,
    note: String,
): ProductInspectionChecklistState =
    copy(
        itemNotesByItemNumber = if (note.isBlank()) {
            itemNotesByItemNumber - itemNumber
        } else {
            itemNotesByItemNumber + (itemNumber to note)
        },
    )

fun ProductInspectionChecklistState.completedItemCount(): Int =
    ratingsByItemNumber.keys.count { itemNumber -> itemNumber in ProductInspectionChecklistCatalog.itemNumbers }

fun ProductInspectionChecklistState.missingItemCount(): Int =
    ProductInspectionChecklistCatalog.totalItemCount - completedItemCount()

fun ProductInspectionChecklistState.isComplete(): Boolean =
    missingItemCount() == 0

private fun List<Pair<Int, String>>.toChecklistItems(): List<ProductChecklistItemDefinition> =
    map { (number, prompt) -> ProductChecklistItemDefinition(number = number, prompt = prompt) }
