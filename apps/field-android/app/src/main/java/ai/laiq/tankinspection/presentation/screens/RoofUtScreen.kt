    ...
    LazyColumn(
        ...
        item {
            LaiqSectionCard(
                ...
                RoofSurfaceMap(
                    ...
                    savedPlateIds = draftState.roofUtRows.map { row -> row.plateId }.toSet(),
                    overlayPlateIds = draftState.roofFeatures.mapNotNull { feature -> feature.plateId }.toSet(),
                    centerFeatureCount = draftState.roofFeatures.count { feature ->
                    ...
                    onSelectPlate = { plateId ->
                        onDraftStateChange(
                            draftState.copy(
                                roofUtDraft = draftState.roofUtDraft.copy(plateId = plateId),
                            ),
                        )
                    },
        ...
        item {
            LaiqSectionCard(
                ...
                subtitle = "Select the numbered roof plate, then capture the five thickness readings for that plate.",
            ) {
                ...
                LaiqDropdownField(
                    ...
                    onSelected = { plateId ->
                        onDraftStateChange(draftState.copy(roofUtDraft = draftState.roofUtDraft.copy(plateId = plateId)))
                    },
                )
                Text(
                ...
                LaiqPrimaryButton(
                    ...
                    onClick = { onDraftStateChange(draftState.saveRoofUtDraft()) },
                )
                LaiqSecondaryButton(
                    ...
                    onClick = {
                        onOpenFindings(
                            draftState.beginFindingForMeasurement(
                                surface = "roof",
                                linkedMeasurementId = draftState.roofUtDraft.editingRowId.orEmpty(),
                                locationSummary = draftState.roofUtDraft.plateId,
                            ),
                        )
                    },
                )
                            ...
                            Surface(
                                ...
                                Column(
                                    ...
                                    Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                                        LaiqSecondaryButton(
                                            text = "Edit",
                                            onClick = { onDraftStateChange(draftState.editRoofUtRow(row.rowId)) },
                                            modifier = Modifier.weight(1f),
                                        )
                                        LaiqSecondaryButton(
                                            text = "Delete",
                                            onClick = { onDraftStateChange(draftState.removeRoofUtRow(row.rowId)) },
                                            modifier = Modifier.weight(1f),
                                        )
                                    }
                                    LaiqSecondaryButton(
