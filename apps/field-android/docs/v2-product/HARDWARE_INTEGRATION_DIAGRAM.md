...
This document refines the future smart-hardware integration plan for the tank inspection product. The target architecture is simple: field hardware can accelerate capture, but the Android V2 Product app remains the canonical field-capture host and the V2 export package remains the report-generation handoff.

## Current Project Anchors

- Android V2 Product stores inspection work locally and exports `packageType: "v2_product_export"` with `schemaVersion: 2`.
- The report platform imports that package through the Android V2 Product import path, then creates a tenant/workspace report job.
...
- Hardware integrations should map into the current V2 export sections first: `inspectionRecord`, `layoutTargets`, `layoutConfigs`, `elements`, `utMeasurements`, `inspectionChecklistItems`, `inspectionChecklistSectionNotes`, `findings`, `attachments`, `taskSnapshots`, and `validationResults`.
...
`apps/field-android/docs/v2-product/diagrams/hardware-integration-diagram.mmd`

Rendered SVG target:

`apps/field-android/docs/v2-product/diagrams/hardware-integration-diagram.svg`
...
flowchart LR
  ...
  subgraph Android["Android V2 Product App"]
    ...
    EXPORTER["V2 Product Exporter<br/>schemaVersion 2"]
  end
  ...
  subgraph Report["Report Platform And AI Engine"]
    IMPORT["Import Adapter<br/>validate V2 package + attachments"]
...
3. Android V2 Product accepts only reviewed/approved capture data and stores it in Room plus local attachment files.
4. Android exports a schema-valid V2 package containing JSON facts, validation results, layout data, findings, UT measurements, checklist data, and copied attachments.
...
| Export handoff | `v2_product_export` JSON plus copied attachments and validation results | Export package file/folder and optional shared/uploaded bundle |
...
| Device family | Field value | Android V2 destination | Report and AI value |
| --- | --- | --- | --- |
...
Purpose:
- Provide a trusted bridge between heterogeneous devices and the Android V2 Product capture model.
...
The current V2 package is intentionally structured and report-friendly. Hardware should extend it through a versioned contract, likely `schemaVersion: 3`, with additions such as:
...
- Should hardware metadata ship in a minor V2 extension or wait for `schemaVersion: 3`?
