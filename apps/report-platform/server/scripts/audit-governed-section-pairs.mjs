...
try{
  ...
  const missingVoice=await db.prepare(`SELECT COUNT(*)::int count FROM report_governed_baseline_pairs WHERE status_code='ready' AND jsonb_array_length(mock_input_json->'voiceNotes')=0`).get();
  ...
  const failures={misaligned:misaligned.count,validationReportInIndex:retrievalLeak.count,validationLineageInIndex:lineageLeak.count,exactGoldInMock:exactGoldLeak.count,missingVoice:missingVoice.count,failedAnswerability:failedAnswerability.count,generatorGoldAccess:generatorGoldAccess.count};
