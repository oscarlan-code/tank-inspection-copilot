    ...
    return {
      reply:
        missingCount === 0
          ...
          : `The missing-content panel has ${missingCount} open item${missingCount === 1 ? "" : "s"} for ${context.template.title}: ${missingLabels.join(", ")}.`,
      actions,
    };
  }
...
function buildMeasurementTableReply(context, query) {
  ...
  const rows = context.exportPackage.utMeasurements
    .filter(
      (measurement) =>
...
function getMeasurementTableScope(sectionId) {
  const scopes = {
    ...
    "shell-nozzle-reinforcement-pad-thickness-measurements": {
      ...
      rowLabel: "Nozzle",
      firstColumnLabel: "Nozzle",
      valueLabels: ["12 o'clock", "3 o'clock", "6 o'clock", "9 o'clock", "Rep. Pad"],
      note: "Shell nozzle rows include reinforcement pad readings where exported.",
    },
    "floor-plate-layout-platemaps-numbering-system": {
      targetKey: "floor",
      itemKind: "region",
      rowLabel: "Floor plate",
      firstColumnLabel: "Plate No.",
      valueLabels: ["A", "B", "C", "D", "E"],
      note: "Floor UT backup readings are exported as floor plate rows.",
    },
  };
    ...
    .map((measurement) => {
      ...
      const statValues =
        includeStats && numericValues.length > 0
          ...
          : [];
      const valueCells = [...displayedValues, ...statValues]
        .map((value) => `<td>${formatThicknessCell(value)}</td>`)
        .join("");

      return `<tr><td>${escapeHtml(rowLabel)}</td>${valueCells}</tr>`;
    })
    .join("");
...
function formatThicknessCell(value) {
...
function buildRequiredManualInputState(context) {
  return context.template.requiredManualFields.map((fieldKey) => {
    ...
    return {
      key: fieldKey,
      label: MANUAL_FIELD_LABELS[fieldKey] ?? humanizeKey(fieldKey),
      value,
