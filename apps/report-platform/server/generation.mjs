...
export async function generateSectionDraft({ reportState, sectionId, userInstruction = "" }) {
  ...
  const content = shouldHighlightPredictedContent({
    context,
...
function buildSectionContext({ reportState, sectionId, userInstruction }) {
  ...
  const factRecommendationPack = useFactRecommendationKb
    ? searchFactRecommendationPairs({ sectionId, reportState })
...
function shouldUseFactRecommendationKb(sectionId, template) {
  return template.kind === "narrative" ||
    sectionId.includes("repair") ||
    sectionId.includes("recommendation") ||
    sectionId.includes("inspection-report");
}
...
function wrapInlineProvenance(kind, value) {
  const normalizedKind = ["app_field_data", "precedent_template", "llm_prediction"].includes(kind)
    ? kind
    : "llm_prediction";
