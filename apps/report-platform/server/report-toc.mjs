...
export const API_STANDARD_REPORT_TOC = [
...
export const API_STANDARD_SHELL_MAP_SECTION_IDS = new Set(
  API_STANDARD_REPORT_TOC
    .filter((section) => section.kind === "map" && section.layoutSurface === "shell")
    .map((section) => section.id),
);

export function getApiStandardTocSection(sectionId) {
  return API_STANDARD_REPORT_TOC.find((section) => section.id === sectionId);
}

export function getApiStandardTocSectionForPage(document, pageNumber) {
  if (!document?.sourceReportName?.includes(API_STANDARD_PRIMARY_REPORT.sourceReportName)) {
    return null;
  }

  return API_STANDARD_REPORT_TOC
    .filter((section) => pageNumber >= section.pageStart)
    .sort((left, right) => right.pageStart - left.pageStart)[0] ?? null;
}
