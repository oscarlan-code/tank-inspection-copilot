import { API_STANDARD_PRIMARY_REPORT } from "./reportToc";
import type { V2ProductExportPackage } from "./v2ProductExport";

export type ReportCodeApplicability = {
  codeId: string;
  label: string;
  role: "primary" | "supporting" | "not_applicable";
  applicability: "applies" | "conditional" | "excluded";
  reason: string;
};

export type ReportClassification = {
  reportFamilyId: string;
  reportFamilyLabel: string;
  inspectionMode: string;
  tankType: string;
  primaryFormatPrecedent: string;
  formatRationale: string;
  primaryCodes: ReportCodeApplicability[];
  supportingCodes: ReportCodeApplicability[];
  excludedCodes: ReportCodeApplicability[];
};
...
export function classifyReportPackage(exportPackage: V2ProductExportPackage): ReportClassification {
  ...
  const primaryCodes: ReportCodeApplicability[] = [
    {
      codeId: "api-653",
      label: "API 653",
      role: "primary",
  ...
  const supportingCodes: ReportCodeApplicability[] = [
    {
      codeId: "eemua-159",
      label: "EEMUA 159",
      role: "supporting",
  ...
  const excludedCodes: ReportCodeApplicability[] = [
    {
      codeId: "api-620",
      label: "API 620",
      role: "not_applicable",
